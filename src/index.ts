...
import { xdr, Operation, Address, Keypair } from "@stellar/stellar-sdk/minimal";
...
export class MyMCP extends McpAgent<Env, unknown, Props> {
	...
	async init() {
		...
		const contractIds = await stub.getContracts(this.props.userEmail);
					...
					async (params) => {
						try {
							...
							if (at.isReadCall) {
								...
								const address_sk = await stub.getAddress(this.props.userEmail);
		...
		this.server.tool(
			...
			async ({ name, address }) => {
				...
				await stub.addContract(this.props.userEmail, name, address);
		...
		this.server.tool(
			...
			{},
			async () => {
				const contracts = await stub.getContracts(this.props.userEmail);
		...
		this.server.tool(
			...
			async ({ address }) => {
				stub.removeContract(this.props.userEmail, address);
		...
		this.server.tool(
			...
			async ({ wallet }) => {
				stub.setAddress(this.props.userEmail, wallet);
